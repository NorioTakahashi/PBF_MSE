...
plot_output <- function(tb, which_item = "", iter_no){
  ...
  title <- if_else(which_item == "rec_devs", paste0("Future generated rec_devs for itr = ", as.character(iter_no)), paste0("Future simulated ", which_item, " with HCR #15 for itr =", as.character(iter_no)))
...
fullpath <- file.path(paste0(my_path, hcr_no[[1]]), as.character(4), "outlist.txt")

tb <- suppressMessages(read_delim(fullpath, delim = " ", skip = 1, col_names = FALSE)) %>% 
  select(-X1)

colnames(tb) <- suppressMessages(suppressWarnings(colnames(read_delim(fullpath, delim = " "))))
