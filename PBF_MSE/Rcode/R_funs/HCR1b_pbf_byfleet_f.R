...
HCR1b_pbf_byfleet_f <- function(ssout, dat, yr, SSBtrs, SSBlim, Ftgt,cr, err, Fmin, hs,hcr,scn,itr,tstep, yrb,yrf){
  ...
  if (SSBcur >= SSBtrs){
    ...
    Fm = ((fmult/(1+exp(-(SSBcur-(((SSBtrs-SSBlim)/2)+SSBlim)))))+(fmult*Fmin))*(fmult/(fmult+Fmin*fmult))
  } else {
    Fm=Fmin*Fmult
  }
