...
PBF_MSE_hs1b = function(hsnum,hcrnum,scnnum,itr, Bthr, Blim, sa, Fmin) { 
...
for (tstep in 1:length(asmt_t)){
  ...
  if (sa==0) {
    ...
    TAC_mat = HCR1b_pbf_byfleet_f(ssout=om_out, dat = SPRmat, yr=yr_end, SSBtrs=ssb_thr, SSBlim=ssb_lim, Ftgt=om_ftgt, cr=cr_all, err=1, Fmin=Fmin,hs=hs,hcr=hcr,scn=scn,itr=itr,tstep=tstep,yrb=c(2002:2004),yrf=c(2017:2019))
    ...
    EM_fun_adj(pdir, sdir, hs, hcr, scn, hsw, hcrw, scnw, pwin, itr, tstep, datatype=3)
    ...
    #Generate TAC based on current harvest control rule
    TAC_mat = HCR1b_pbf_byfleet_f(ssout=em_out, dat = SPRmat, yr=yr_end, SSBtrs=ssb_thr, SSBlim=ssb_lim, Ftgt=om_ftgt, cr=cr_all, err=1, Fmin=Fmin,hs=hs,hcr=hcr,scn=scn,itr=itr,tstep=tstep,yrb=c(2002:2004),yrf=c(2017:2019))
